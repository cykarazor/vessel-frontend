...
function App() {
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "2rem auto", fontFamily: "sans-serif" }}>
      <h1>Voyages</h1>
      <button style={{ marginBottom: 12 }} onClick={openAddModal}>Add Voyage</button>
      <table border="1" cellPadding={6} cellSpacing={0} style={{ width: "100%", background: "#fff" }}>
        ...
        <tbody>
          {voyages.map(v => (
            <tr key={v._id} style={{ cursor: "pointer" }} onClick={() => openModal(v)}>
              <td>{v.vesselName}</td>
      ...
      <Modal open={!!selectedVoyage} onClose={closeModal}>
        {editMode ? (
          <form onSubmit={handleSubmit}>
            <h2>{selectedVoyage && selectedVoyage._id ? "Edit Voyage" : "Add Voyage"}</h2>
            ...
            <div style={{ marginTop: 16 }}>
              <button type="submit">{selectedVoyage && selectedVoyage._id ? "Update" : "Add"}</button>
